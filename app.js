...
app.controller('appCtrl', ['$scope', 'FirebaseIndexView', '$timeout', function ($scope, FirebaseIndexView, $timeout) {
    ...
    function convertToCase(doc, index, upper) {
        if (doc[index]) return upper ? doc[index].toUpperCase() : doc[index].toLowerCase();
    }

    function convertToBirthday(doc) {
        if (doc.birthdate) {
            var a = new Date(doc.birthdate).getTime();
            return Math.floor(a / 36000000) + 100000;
        }
    }

    function convertToBirthdayQuery(term) {
        var a = new Date(term).getTime();
        return Math.floor(a / 36000000) + 100000;
    }

    function emitFullArray(doc, specify, stamp) {
        var c = 0;
        return [{
            count: c++,
            name: doc.lastname
    ...
    function emitFullName(doc, specify, stamp) {
        return {
            ...
            childrens: (function () {
                ...
                if (doc.children) {
                    for (var f = 0; f < doc.children.length; f++) {
                        r.push(doc.children[f].firstname);
                    }
                }
                return r;
            })(),
    ...
    var indexDeclaration_karteixxxxxxxxx = [
        {
            ...
            mapIndex: function (doc) {
                return FirebaseIdx.getLinkedIndex(doc, "_linkedX");
            },
            mapView: function (doc) {
                return FirebaseIdx.getLinkedDocument(doc, "_linkedX");
            }
        },
        {
            indexName: 'diagnose',
            mapIndex: function (doc) {
                return (doc.diagnose && doc.diagnose.length) ? doc.diagnose[0].toLowerCase() : undefined;
            },
            mapView: function (doc) {
                return doc.diagnose.join(",");
            }
        },
        {
            indexName: 'diagnose',
            mapIndex: function (doc) {
                return (doc.diagnose && doc.diagnose.length) ? doc.diagnose[1].toLowerCase() : undefined;
            },
            mapView: function (doc) {
                return doc.diagnose.join(",");
            }
        },
        {
            indexName: 'diagnose',
            mapIndex: function (doc) {
                return (doc.diagnose && doc.diagnose.length) ? doc.diagnose[2].toLowerCase() : undefined;
            },
            mapView: function (doc) {
                return doc.diagnose.join(",");
            }
        },
        {
            indexName: 'leistung',
            mapIndex: function (doc) {
                return (doc.leistung && doc.leistung.length) ? doc.leistung[0].toLowerCase() : undefined;
            },
            mapView: function (doc) {
                return doc.leistung.join(",");
            }
        },
        {
            indexName: 'leistung',
            mapIndex: function (doc) {
                return (doc.leistung && doc.leistung.length) ? doc.leistung[1].toLowerCase() : undefined;
            },
            mapView: function (doc) {
                return doc.leistung.join(",");
            }
        },
        {
            indexName: 'leistung',
            mapIndex: function (doc) {
                return (doc.leistung && doc.leistung.length) ? doc.leistung[2].toLowerCase() : undefined;
            },
            mapView: function (doc) {
                return doc.leistung.join(",");
            }
    ...
    var indexDeclaration_person = [
        {
            ...
            mapView: function (doc) {
                return (doc.firstname && doc.lastname) ? doc.firstname + ' ' + doc.lastname : "xxx";
            }
        },
    /*
    ...
    var indexDeclaration = [
        ...
        {
            indexName: 'dino_orderweight',
            mapIndex: function (doc) {
                return doc.order ? doc.order.toLowerCase() : '';
            },
            mapView: function (doc) {
                return (doc.order && doc.weight) ? doc.order + ' has weight of ' + doc.weight : "no Dinosaur";
            }
        },
        {
            indexName: 'dino_dimension',
            mapIndex: function (doc) {
                return doc.length && doc.weight ? (doc.length * doc.weight) : null;
            },
            mapView: function (doc) {
                return (doc.length && doc.weight) ? doc._key + ' has a volume from  ' + (doc.length * doc.weight) : "no volume";
            }
        },
        {
        ...
        },
        {
            ...
            mapView: function (doc) {
                    return (doc.order === 'ornithischia') ? true : null;
                } // if ornithischia flying is true else generate no index
        },
    ...
    function mapIndexMeal(doc, kind) {
        if (doc.meals) {
            return doc.meals[kind] ? doc.meals[kind].toLowerCase() : null;
        }
    }

    function mapViewMeal(doc, kind, action) {
        return doc.meals ? (doc.meals[kind] ? ((doc._linked + " " + action + " " + doc.meals[kind]) || 'no ' + kind) : null) : null;
    }
    ...
    var indexDeclaration_kartei = [
        {
            ...
            mapIndex: function (doc) {
                return FirebaseIdx.getLinkedIndex(doc) || null;
            },
            mapView: function (doc) {
        ...
        {
            indexName: 'meals/onlydrinks',
            mapIndex: function (doc) {
                if (doc.meals) {
                    return doc.meals.drink ? doc.meals.drink.toLowerCase() : null;
                }
            },
            mapView: [mapViewMeal, 'drink', 'drinks']
        },
        {
            /*
            ...
            */
            indexName: 'meals/day',
            mapIndex: function (doc) {
                return doc.mealsPerDay || null;
            },
            mapView: function (doc) {
                var dino = FirebaseIdx.getLinkedDocument(doc);
                return doc.mealsPerDay + " meals per day with " + doc.calory + " calories/meal make a dino weight of " + (dino ? dino.weight : null);
            }
        },
    ...
    var FirebaseIdx_kartei = new FirebaseIndexView(reference_kartei, indexDeclaration_kartei); // take Default referenceIndexView
    // alternativ  
    // var FirebaseIdx=new FirebaseIndexView(reference, indexDeclaration, referenceIndexView ); // use referenceIndexView
    ...
    FirebaseIdx = new FirebaseIndexView(reference, indexDeclaration, referenceIndexView); // take Default referenceIndexView
    FirebaseIdx_kartei = new FirebaseIndexView(reference_kartei, indexDeclaration_kartei, referenceIndexView_kartei); // take Default referenceIndexView
    ...
    /*
        ...
        -
        FirebaseIndexViewCompany.update(referenceIndexView_Person.child('_joinPerson'));
        or 
        FirebaseIndexViewCompany.update(referenceIndexView_Person, '_joinPerson');
   ...
   */
    ...
    */
    ...
    $scope.query = function (term, index, sort, view) {
        ...
        console.log(termFrom, termTo, sort, view);


        FirebaseIdx_kartei.queryFromTo(index, termFrom, termTo, sort, view)
            //FirebaseIdx.queryFromTo(index, termFrom)        
            //FirebaseIdx_kartei.queryStartAt(index, termFrom, 999,sortup, view)        
            .then(function (result) {
                return FirebaseIdx.queryFromTo(index, termFrom, termTo, sort, view, result);
            })
            .then(function (result) {
                //console.log(result);
                $timeout(function () {
    ...
    function generateDinoMeal() {
        ...
        var m = ["Beef", "Sausage", "Lamb", "Turkey", "Dino small", "Dino XXL", "rotten Carcass", "Humans", "Mammoth"];
        if (Math.random() > 0.2) ml.meat = getRandom(m);

        var v = ["gras", "fruits", "bananas", "trees", "apples", "grain", "leafs", "straw", "corn"];
        if (Math.random() > 0.25) ml.vegetarian = getRandom(v);

        var dx = ["Water", "Milk", "blood", "Beer", "seawater", "Budweiser", "Fruit Juice"];
        ml.drink = getRandom(dx);

        p.meals = ml;

        p.calory = 100 + Math.floor(Math.random() * 5000);
