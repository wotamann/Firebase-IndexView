...
app.controller('appCtrl', ['$scope', 'FirebaseIndexView', '$timeout', function ($scope, FirebaseIndexView, $timeout) {
    ...
    var indexDeclarationDinosaur = [
        ...
        {   
            viewName: 'isFlying',
            mapView: function(emit, doc) {
                if ((doc.order==='ornithischia')===true) emit(doc._key,true);
            }            
        },
    ...
    var IndexViewDino = FirebaseIndexView(referenceDinosaur, indexDeclarationDinosaur, referenceDinosaurIDX); // take Default referenceIndexView
    ...
    var indexDeclarationDinomeals = [
        ...
        {   
            ...
            mapView:function(emit, doc) {
                ...
                var key=doc._linked || null,
                    ext_doc=IndexViewDino.getLinkedDocument(doc._linked);
                    if( ext_doc) {ext_doc.food=doc}; 
                    var val= ext_doc || 'no dino found';
                emit(key,val);
            }   
    ...
    var IndexViewDinoMeals = FirebaseIndexView(referenceDinoMeal, indexDeclarationDinomeals); // take Default referenceIndexView
    ...
    function countMealsFN(previousValue, currentValue, index, array) {
            ...
            if (index===1) previousValue={drink:0,meat:0,vegetarian:0};
            //console.log(index, previousValue,currentValue)
            if(currentValue.drink) previousValue.drink=previousValue.drink + currentValue.drink.split(",").length
            if(currentValue.meat) previousValue.meat=previousValue.meat + currentValue.meat.split(",").length
            if(currentValue.vegetarian) previousValue.vegetarian=previousValue.vegetarian + currentValue.vegetarian.split(",").length
    ...
    function generateDinoMeal() {
        ...
        if (Math.random() > 0.4) ml.meat += getRandom(m)+",";
        if (Math.random() > 0.4) ml.vegetarian += getRandom(v)+",";
        if (Math.random() > 0.4) ml.drink += getRandom(dx)+",";
        
        if (Math.random() > 0.66) ml.meat += getRandom(m)+",";
        if (Math.random() > 0.66) ml.vegetarian +=  getRandom(v)+",";
        if (Math.random() > 0.66) ml.drink += getRandom(dx)+",";

        if (Math.random() > 0.8) ml.meat += getRandom(m)+",";
        if (Math.random() > 0.8) ml.vegetarian +=  getRandom(v)+",";
        if (Math.random() > 0.8) ml.drink += getRandom(dx)+",";
